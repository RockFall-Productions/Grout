...
namespace Grout {
    ...
    void Mesh::SetupMesh()
    {
        ...
        vertex_buffer.reset(VertexBuffer::Create(&vertices[0], vertices.size() * sizeof(Vertex)));
        BufferLayout layout = { 
        ...
        index_buffer.reset(IndexBuffer::Create(&indices[0], indices.size()));
    ...
    std::vector<Mesh::Texture> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                texture.id = Mesh::TextureFromFile(str.C_Str(), this->directory, gamme_correction_);
